...
from models import cifar_resnet, cifar_vgg, mnist_lenet, imagenet_resnet, catdog_vgg
...
def get(model_hparams: ModelHparams, outputs=None):
    ...
    if hasattr(bn_initializers, model_hparams.batchnorm_init):
        ...
        raise ValueError(
            "No batchnorm initializer: {}".format(model_hparams.batchnorm_init)
        )
    ...
    for k, v in model.named_parameters():
        if k in bn_names and model_hparams.batchnorm_frozen:
            ...
            k not in bn_names
            and k not in model.output_layer_names
            and model_hparams.others_frozen
        ):
            if k in others_exception_names:
                continue
            v.requires_grad = False
